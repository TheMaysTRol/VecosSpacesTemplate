...
#pragma warning restore 0168
        ...
        public static int MinViewIdForScene(string sceneName)
        {
            ...
            {
                Debug.Log(pss.MinViewIdPerScene.Find(x => x.sceneName == scene.name));
                if (pss.MinViewIdPerScene.Find(x => x.sceneName == scene.name) == null)
                {
                    var sceneAsset = AssetDatabase.LoadAssetAtPath<SceneAsset>(scene.path);
                    var set = new SceneSetting();
                    set.sceneAsset = sceneAsset;
                    set.sceneName = sceneAsset.name;
                    set.minViewId = 100;
                    pss.MinViewIdPerScene.Add(set);
                }
            };
#endif
