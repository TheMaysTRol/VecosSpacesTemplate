...
#pragma warning restore 0414
        ...
        /// </summary>
        internal static void ExecuteRpc(Hashtable rpcData, Player sender, int retries = 0)
        {
            ...
            if (photonNetview == null)
            {
                ...
                if (AddressablesLoader.Instance)
                {
                    Debug.LogError("Photon view is null");
                    AddressablesLoader.Instance.ReExectueRPC(rpcData, sender, retries);
                }
                return;
            }

            Debug.LogError("RPC " + inMethodName + " Called for  " + photonNetview.name + photonNetview.ViewID);
                ...
                case PunEvent.OwnershipRequest:
                    {
                        ...
                        if (requestedView == null)
                        {
                            ...
                            {
                                Debug.LogError("Photon view is null");
                                AddressablesLoader.Instance.ReExectueEvent(photonEvent, 0);
                            }
        ...
        public static void OnEvent2(EventData photonEvent, int retries = 0)
        {
            int actorNr = photonEvent.Sender;
            Player originatingPlayer = null;
            if (actorNr > 0 && NetworkingClient.CurrentRoom != null)
                ...
                case PunEvent.OwnershipRequest:
                    {
                        ...
                        if (requestedView == null)
                        {
                            ...
                            {
                                Debug.LogError("Photon view is null");
                                AddressablesLoader.Instance.ReExectueEvent(photonEvent, retries);
                            }
                            Debug.LogWarning("Can't find PhotonView of incoming OwnershipRequest. ViewId not found: " + requestedViewId);
                            break;
                        }
